...
- Напишите в консоли (при этом консоль должна быть открыта в папке `startjava`): 
  ...
  - `git remote add origin url_на_ваш_startjava-репозиторий.git` — добавляет, созданный вами репозиторий в git
...
8. `git add Файл_с_кодом.java` (файлы с расширением class не заносите) — добавляет файлы в git
9. `git commit -m "описание изменений, которые вы внесли в программу"` — фиксирует изменения кода в git
10. `git push -u origin master` — это действие делайте каждый раз, когда хотите отправить свой код на github
