...
- Модифицируйте класс `Jaeger`, сделав его универсальным (без инициализации полей в самом классе, как мы это делали в уроке). Это позволит на его основе создавать любого робота:
  ...
  - выберите два или более разных роботов. Найдите между ними что-то общее. Реализуйте это в коде
  - создайте 2 экземпляра класса `Jaeger` и поэксперементируйте с ними (проинициализируйте поля, вызывайте методы, перезаписывайте и выводите значения полей)
  
 ## Советы для тех, кто выполняет домашнее задание
 1. для инициализации полей объектов при их создании используйте:
    - в начале сеттеры
    - а потом перепишите, используя [конструктор](https://vertex-academy.com/tutorials/ru/konstruktory-slovo-v-java/)
 2. Для ввода чисел (в `Калькуляторе` и `Угадай число`) и знака математической операции используйте клавиатуру (класс [`Scanner`](https://vertex-academy.com/tutorials/ru/rabota-so-skannerom-v-java/))
 3. Формат ввода математического выражения для программы `Калькулятор` (пример):  
    `Введите первое число: 10`   
    `Введите знак математической операции: +`  
    `Введите второе число: 10`
 4. Метод `main` необходимо размещать в классах, в имени которых присутствует слово `Test`
 5. Назначение классов в игре `Угадай число`:
 ...
 6. Для завершения или продолжения работы программ выводите сообщение: `"Хотите продолжить? [да/нет]: "`
 7. Если пользователь ввел ни "да" ни "нет", а что-то другое — снова выведите сообщение из п.5
 8. Не забывайте указывать для полей и методов модификаторы доступа
 9. Класс `Player` должен иметь следующие поля и методы:
    - `String name` — имя игрока (**одного!**)
    - `int number` — введенное пользователем число
    - геттеры/сеттеры
    - метод для ввода пользователем числа
 10. Примерно так должна выглядеть структура ваших файлов и папок
