...
- Покажите наставнику:
    ...
    - скрин с выводом дерева каталогов и файлов, начиная от папки `stratjava` (для этого используйте в консоли `tree /F`)

## Советы для тех, кто выполняет домашнее задание
- Не забудьте файл `.gitignore` добавить в git (с помощью `add`)
- `-sourcepath имя_дериктории` — данный ключ позволяет упростить компиляцию программы, состоящей из нескольких файлов
